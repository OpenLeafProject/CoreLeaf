...
namespace Leaf.Controllers
{
    ...
    public class UserProfileController : ControllerBase
    {
        ...
        public UserProfileController(ILogger<UserProfileController> logger, IConfiguration config)
        {
            _logger = logger;
            _config = config;
        }
        ...
        public ActionResult<object> Get(string id)
        {
            ...
            try
            {
                ...
                {
                    return new UserProfile(code, _config);
                }
                else
                {
                    return new UserProfile(id, _config);
                }
        ...
        public ActionResult<object> Create([FromBody] Dictionary<string, string> values)
        {
            try
            {
               return new UserProfile(values,_config).Create();
            }
            catch (Exception ex)
            {
                return BadRequest(new Dictionary<string, string>() {
        ...
        public ActionResult<object> Save([FromBody] Dictionary<string, string> values)
        {
            try
            {
                return new UserProfile(values, _config).Save();
