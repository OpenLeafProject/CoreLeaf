...
namespace Leaf.Controllers
{
    ...
    {
        private readonly ILogger<VisitModeController> _logger;
        ...
        public VisitModeController(ILogger<VisitModeController> logger, IConfiguration config)
        {
            _logger = logger;
            _config = config;
        }
        ...
        public ActionResult<object> Get(string id)
        {
            ...
            try
            {
                ...
                {
                    return new VisitMode(code, _config);
                }
                else
                {
                    return new VisitMode(id, _config);
                }
        ...
        public ActionResult<object> Create([FromBody] Dictionary<string, string> values)
        {
            try
            {
               return new VisitMode(values,_config).Create();
            }
            catch (Exception ex)
            {
                return BadRequest(new Dictionary<string, string>() {
        ...
        public ActionResult<object> Save([FromBody] Dictionary<string, string> values)
        {
            try
            {
                return new VisitMode(values, _config).Save();
